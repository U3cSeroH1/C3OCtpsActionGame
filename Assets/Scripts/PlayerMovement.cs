...
public class PlayerMovement : MonoBehaviour
{
    ...
    */
    ...
    void Update()
    {
        ...
        //movement = Vector3.Scale( movement , Camera4Direction.transform.forward);
    }
    ...
    public void AddforceExec(Vector3 power, ForceMode forceMode)
    {
        //Debug.Log (Vector3.Scale(power, Camera4Direction.transform.forward));

        rb.AddForce(power, forceMode);
    }
    ...
    public void caluclateMovement()
    {
        ...
        if (grounded && doJump != 3)
        {
            ...
            AddforceExec(new Vector3(movement.x, movement.y, movement.z), ForceMode.VelocityChange);
        ...
        else
        {
            ...
            if (groundedLastFrame && doJump != 3 && !falling)
            {
                ...
                if (Physics.Raycast(transform.position, Vector3.down, out hit, fudgeCheck + (rb.velocity.magnitude * Time.deltaTime), ~0) && Vector3.Angle(hit.normal, Vector3.up) <= MaximumSlope)
                {
                    ...
                    if (Physics.Raycast(new Vector3(transform.position.x, transform.position.y - bottomCapsuleSphereOrigin, transform.position.z), -hit.normal, out hit, hit.distance, ~0))
                    {
                        ...
                        AddforceExec(hit.normal * -hit.distance, ForceMode.VelocityChange);
            ...
            if (!Mathf.Approximately(inputX + inputY, 0.0f))
            {
                ...
                if (length > 0.0f)
                {
                    ...
                    AddforceExec(new Vector3(movement.x, 0.0f, movement.z), ForceMode.VelocityChange);
